...
const CountryDropdown = () => {
  ...
  const selectProvince = (index, province) => {
    setSelectedTab(index);
    setIsOpenModal(false);
    context.setSelectedCountry(province) // Store the selected province
  }
  ...
  useEffect(() => {
    ...
    axios.get("https://esgoo.net/api-tinhthanh/1/0.htm")
      .then((res) => {
        const provinces = res.data.data;  // New API structure
        setProvinceList(provinces);
        setOriginalProvinceList(provinces);
      })
      .catch((error) => {
  ...
  const filterList = (e) => {
    const keyword = e.target.value.toLowerCase();
    const filteredList = originalProvinceList.filter((item) => {
      return item.name.toLowerCase().includes(keyword);  // Filter by province name
    });
      ...
      <Dialog open={isOpenModal} onClose={() => setIsOpenModal(false)} className='locationModel' TransitionComponent={Transition}>
        ...
        <ul className='countryList mt-3'>
          {provinceList?.length !== 0 && provinceList?.map((item, index) => {
            return (
              <li key={index}><Button onClick={() => selectProvince(index, item.name)} 
                className={`${selectedTab === index ? 'active' : ''}`}
              >{item.name}</Button></li>
            );
          })}
